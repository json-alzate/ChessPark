...
export class BlockService {
    ...
    return new Promise((resolve, reject) => {
      ...
      switch (option) {
        ...
        case 'plan1': // No muestra soluciones / un mismo color / 1 minuto de short
          ...
          const shortElo1 = profile?.elos?.plan1 ? profile?.elos?.plan1['short'] : undefined;
  ...
  getWeaknessInPlan(plan: {
    [key: string]: number;
  }, allowedThemeValues?: string[]): string {
    ...
    Object.keys(plan).forEach(key => {
      ...
      if (allowedThemeValues?.length && !allowedThemeValues.includes(key)) return;
