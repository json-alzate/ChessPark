...
export class StorageService {
  private readonly STORAGE_KEY = 'chessColate_coordinates_games';
  private readonly USER_ID_KEY = 'chessColate_coordinates_user_id';
  ...
  getUserStats(): {
    ...
    const totalCorrect = games.reduce(
      (sum, game) => sum + game.squaresGood.length,
      0
    );
    const totalIncorrect = games.reduce(
      (sum, game) => sum + game.squaresBad.length,
      0
    );
